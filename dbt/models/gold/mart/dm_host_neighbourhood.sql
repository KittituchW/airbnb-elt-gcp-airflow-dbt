...
host_month AS (
  SELECT
    m.month_start::date AS month_start,
    h.host_id,
    UPPER(TRIM(h.host_neighbourhood))                   AS host_neighbourhood,
    CASE
...
-- Distinct-host based superhost rate per host_neighbourhood
host_flags AS (
  SELECT
    host_neighbourhood,
    month_start,
...
agg AS (
  ...
  FROM base b
  JOIN host_flags hf
    ON hf.host_neighbourhood = b.host_neighbourhood
   AND hf.month_start        = b.month_start
  GROUP BY b.host_neighbourhood, b.month_start
),
with_changes AS (
  SELECT
    a.*,
    (a.listings_active::numeric / NULLIF(a.listings_total, 0)) AS active_listings_rate,
    (a.listings_active - LAG(a.listings_active) OVER (PARTITION BY a.host_neighbourhood ORDER BY a.month_start))
      / NULLIF(LAG(a.listings_active) OVER (PARTITION BY a.host_neighbourhood ORDER BY a.month_start), 0)::numeric
                                                                  AS pct_change_active,
    (a.listings_inactive - LAG(a.listings_inactive) OVER (PARTITION BY a.host_neighbourhood ORDER BY a.month_start))
      / NULLIF(LAG(a.listings_inactive) OVER (PARTITION BY a.host_neighbourhood ORDER BY a.month_start), 0)::numeric
