...
WITH listing_month AS (
  SELECT
    ...
    l.listing_id,
    l.host_id,
    UPPER(TRIM(l.property_type))                      AS property_type,
...
-- Superhost rate by DISTINCT hosts per property_type
host_flags AS (
  SELECT
    property_type,
    month_start,
...
agg AS (
  ...
  FROM base b
  JOIN host_flags hf
    ON hf.property_type = b.property_type
   AND hf.month_start   = b.month_start
  GROUP BY b.property_type, b.month_start
),
with_changes AS (
  SELECT
    a.*,
    (a.listings_active::numeric / NULLIF(a.listings_total, 0)) AS active_listings_rate,
    (a.listings_active - LAG(a.listings_active) OVER (PARTITION BY a.property_type ORDER BY a.month_start))
      / NULLIF(LAG(a.listings_active) OVER (PARTITION BY a.property_type ORDER BY a.month_start), 0)::numeric
                                                                  AS pct_change_active,
    (a.listings_inactive - LAG(a.listings_inactive) OVER (PARTITION BY a.property_type ORDER BY a.month_start))
      / NULLIF(LAG(a.listings_inactive) OVER (PARTITION BY a.property_type ORDER BY a.month_start), 0)::numeric
